...
public class CategoryDeleteController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			String slugCategory = SlugProcessor.getParam(request.getRequestURI());
			Category category = CategoryDAO.getCategoryBySlug(slugCategory);
			if (category != null) {
				if (CategoryDAO.delete(category)) {
					request.getRequestDispatcher("/admin/danhsachtheloai/" + page).forward(request, response);
					return;
				}
				request.setAttribute("error", "Không xóa được ");
				request.setAttribute("link", "admin/danhsachtheloai/" + page);
				request.getRequestDispatcher("/admin/errorpage.jsp").forward(request, response);
				return;
			}
			request.setAttribute("error", "Thể loại không tồn tại");
			request.setAttribute("link", "admin/danhsachtheloai/" + page);
			request.getRequestDispatcher("/admin/errorpage.jsp").forward(request, response);
			return;
		} catch (Exception e) {
